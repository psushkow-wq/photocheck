"""
ui/result_tabs.py — Result panel with tabbed views for every analysis dimension.

Each tab is a scrollable frame populated after analysis completes.
Structured for easy migration to PyQt: every tab is a standalone method
that receives the AnalysisResult and populates its container.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Public: build_result_tabs
# ─────────────────────────────────────────────────────────────────────────────

def build_result_tabs(notebook: ctk.CTkTabview, result: AnalysisResult) -> None:
    """Populate all result tabs from a completed AnalysisResult."""
    _build_summary_tab(notebook.tab("Итог"), result)
    _build_internet_tab(notebook.tab("Интернет"), result)
    _build_ai_tab(notebook.tab("AI-анализ"), result)
...
def _build_summary_tab(frame: ctk.CTkFrame, result: AnalysisResult) -> None:
    ...
    ctk.CTkLabel(
        ...
        text="⚠️  Это предварительный OSINT/forensic-анализ, не судебная экспертиза.\n"
             "Результаты носят вероятностный характер и не являются доказательством подлинности или фальсификации.",
        text_color=YELLOW, font=FONT_SMALL, wraplength=600, justify="left", anchor="w",
    ).pack(padx=14, pady=10)


# ─────────────────────────────────────────────────────────────────────────────
# Tab 2 — Internet matches
# ─────────────────────────────────────────────────────────────────────────────
...
def _build_internet_tab(frame: ctk.CTkFrame, result: AnalysisResult) -> None:
    ...
    if not rs:
        _label(scroll, "Обратный поиск не выполнялся (режим local или не дано согласие).",
               color=MUTED).pack(pady=20)
        return
    ...
    if rs.earliest_found_url:
        ...
        _label(inner, rs.earliest_found_url or "", color=GREEN, font=FONT_SMALL).pack(fill="x")
    ...
    for match in rs.matches:
        ...
        if match.error:
            status_text = f"⚠️ Ошибка: {match.error}"
            status_color = RED
        ...
        ctk.CTkLabel(header, text=match.service, text_color=svc_color,
        ...
        if match.url and match.url.startswith("http"):
            ...
            short = match.url[:80] + ("…" if len(match.url) > 80 else "")
        ...
        if match.page_title:
            _label(inner, match.page_title, color=MUTED, font=FONT_SMALL).pack(fill="x")
        if match.first_seen_date:
            _label(inner, f"Дата: {match.first_seen_date}", color=MUTED,
                   font=FONT_SMALL).pack(fill="x")
        if match.similarity_score is not None:
            _label(inner, f"Сходство: {match.similarity_score:.0%}", color=MUTED,
                   font=FONT_SMALL).pack(fill="x")
...
def _build_ai_tab(frame: ctk.CTkFrame, result: AnalysisResult) -> None:
    ...
    if ai.service_results:
        ...
        for svc in ai.service_results:
            ...
            if svc.error:
                _label(inner, f"Ошибка: {svc.error}", color=RED, font=FONT_SMALL).pack(fill="x")
            else:
                _label(inner, svc.verdict, color=TEXT, font=FONT_SMALL).pack(fill="x")
    ...
    if ai.local_heuristics_flags:
        ...
        for flag in ai.local_heuristics_flags:
            _label(inner, f"• {flag}", color=MUTED, font=FONT_SMALL).pack(fill="x", pady=1)
        _label(inner,
               "\nЭвристики — это мягкие сигналы, не доказательство AI-генерации.",
...
def _build_metadata_tab(frame: ctk.CTkFrame, result: AnalysisResult) -> None:
    ...
    for label, value in fields:
        ...
        ctk.CTkLabel(row, text=str(value), text_color=color,
    ...
    if m.gps:
        ...
        if m.gps.reverse_geocode_address:
            _label(inner, f"Адрес:    {m.gps.reverse_geocode_address}",
                   font=FONT_SMALL).pack(fill="x")
    ...
    # Raw EXIF dump (collapsible via scrollable)
    if m.raw_exif:
...
def _build_forensics_tab(frame: ctk.CTkFrame, result: AnalysisResult) -> None:
    ...
    if f.ela:
        ...
        if f.ela.ela_image_path and Path(f.ela.ela_image_path).exists():
            try:
                ...
                img_lbl.image = ela_tk  # keep reference
                img_lbl.pack(pady=8)
            except Exception:
                _label(inner, f"ELA изображение: {f.ela.ela_image_path}",
                       color=MUTED, font=FONT_SMALL).pack(fill="x")

        _label(inner,
               "\nELA является эвристикой и может давать ложные срабатывания на изображениях\n"
               "с очень низким или высоким JPEG-качеством. Это индикатор, не доказательство.",
               color=MUTED, font=FONT_SMALL).pack(fill="x", pady=(6, 0))
    ...
    if h:
        ...
        if h.local_duplicate_found:
            _label(card,
                   f"⚠️  Локальный дубликат: {h.local_duplicate_path}  "
...
def _build_report_tab(frame: ctk.CTkFrame, result: AnalysisResult) -> None:
    ...
    ctk.CTkLabel(
        desc,
        text="Сохраните результаты анализа в удобном формате.",
        text_color=MUTED, font=FONT_SMALL, anchor="w",
